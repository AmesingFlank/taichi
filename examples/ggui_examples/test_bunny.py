...
scene = pywavefront.Wavefront(str(pathlib.Path(__file__).parent)+"/bunny.obj",collect_faces=True)
...
while window.running:
    ...
    if with_normals:
        if use_random_colors:
            scene.mesh(vertices,indices,per_vertex_color=colors,shininess = shininess, normals = normals)
        else:
            scene.mesh(vertices,indices,color = (0,0,1),shininess = shininess,normals = normals)
    else:
        if use_random_colors:
            scene.mesh(vertices,indices,per_vertex_color=colors,shininess = shininess)
        else:
