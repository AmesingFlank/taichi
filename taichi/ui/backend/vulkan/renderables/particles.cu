...
void Particles::update_ubo(glm::vec3 color,bool use_per_vertex_color,float radius,float shininess,const Scene& scene) {
...
void Particles::update_data(const ParticlesInfo& info,const Scene& scene){
    ...
    if(info.renderable_info.vertices.matrix_rows != 3 || info.renderable_info.vertices.matrix_cols != 1){
        throw std::runtime_error("Particles vertices requres 3-d vector fields");
    } 

    Renderable::update_data(info.renderable_info);
    
    update_ubo(info.color,info.renderable_info.per_vertex_color.valid,info.radius,info.shininess,scene);
}
...
void  Particles::create_descriptor_set_layout()  {
    ...
    ubo_layout_binding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT | VK_SHADER_STAGE_GEOMETRY_BIT;
