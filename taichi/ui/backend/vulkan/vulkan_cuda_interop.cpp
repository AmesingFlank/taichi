...
void * get_memory_pointer(VkDeviceMemory mem, VkDeviceSize size, VkDevice device) {
    auto handle = get_device_mem_handle(mem,device);
    CUexternalMemory externalMem = import_vk_memory_object_from_handle(handle,size,false);
...
// https://github.com/NVIDIA/cuda-samples/tree/master/Samples/vulkanImageCUDA    // this link uses CUDA runtime api. 
// https://stackoverflow.com/questions/55424875/use-vulkan-vkimage-as-a-cuda-cuarray   // this link contains the necessary information on how to migrate to CUDA driver api

CUsurfObject get_image_surface_object_of_external_memory(CUexternalMemory external_mem, int W,int H){
    ...
    external_mem_mipmapped_array_desc.arrayDesc.Flags = CUDA_ARRAY3D_COLOR_ATTACHMENT;
...
HANDLE get_vk_semaphore_handle(VkExternalSemaphoreHandleTypeFlagBitsKHR external_semaphore_handle_type,VkSemaphore& semaphore,VkDevice device) {
    ...
    semaphore_get_win32_handle_info.handleType =
...
void cuda_vk_semaphore_signal(CUexternalSemaphore ext_smaphore, CUstream stream) {
...
#ifdef _WIN64
    external_semaphore_handle_desc.type = CU_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUE_WIN32;
    external_semaphore_handle_desc.handle.win32.handle = get_vk_semaphore_handle(VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUE_WIN32_BIT,semaphore,device);
#else
    external_semaphore_handle_desc.type = CU_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUE_FD;
    external_semaphore_handle_desc.handle.fd = get_vk_semaphore_handle(VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUE_FD_BIT, semaphore,device);
#endif
