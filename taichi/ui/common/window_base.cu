...
WindowBase :: WindowBase(AppConfig config):config_(config){
    glfw_window_ = create_glfw_window_(config_.name,config_.width,config_.height,config_.vsync);
...
void WindowBase::set_callbacks(){
    ...
    input_handler_.user_mouse_button_callbacks.push_back([&](int key, int action){
        if(action == GLFW_PRESS){
            events_.push_back({EVENT_PRESS,button_id_to_name(key)});
        }
    });
...
void WindowBase::show(){
    ...
    if(current_time-last_record_time_>=1){
        double FPS = (double)frames_since_last_record_/(current_time-last_record_time_);
        std::string glfw_window_text = config_.name + "  " + std::to_string(FPS) + " FPS";
...
void WindowBase::key_callback(GLFWwindow* glfw_window, int key, int scancode, int action, int mode){
    auto window = reinterpret_cast<WindowBase*>(glfwGetWindowUserPointer(glfw_window));
    window->input_handler_.key_callback(glfw_window, key, scancode, action, mode);
}

void WindowBase::mouse_pos_callback(GLFWwindow* glfw_window, double xpos, double ypos){
    auto window = reinterpret_cast<WindowBase*>(glfwGetWindowUserPointer(glfw_window));
    window->input_handler_.mouse_pos_callback(glfw_window, xpos,ypos);
}

void WindowBase::mouse_button_callback(GLFWwindow* glfw_window, int button, int action, int modifier){
    auto window = reinterpret_cast<WindowBase*>(glfwGetWindowUserPointer(glfw_window));
    window->input_handler_.mouse_button_callback(glfw_window, button,action,modifier);
}